...
logger = get_my_logger("crawler_1")

# 음악 파일의 이름과 데이터를 저장하기 위한 namedtuple
...
def download(url, timeout=180):
    ...
    # 음악 파일 내려받기. get request 이후 아무 응답이 없으면 180초 뒤, timeout exception 발생
    r = requests.get(url, timeout=timeout)

    # 내려받기 종료를 로그에 출력
    logger.info(f"[download finished] {file_name}")

    # namedtuple에 파일 이름과 mp3 데이터를 넣어 반환
