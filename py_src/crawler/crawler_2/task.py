...
app = Celery("crawler_2.task", broker="redis://localhost:6379/0")
app.conf.update(
    # Redis에 태스크 또는 실행 결과를 저장할 때의 형식을 JSON으로 지정x
    ...
    task_routes={
        "crawler_2.task.download": {
            "queue": "download",
            "routing_key": "download",
        },
        "crawler_2.task.cut_mp3": {
            "queue": "media",
            "routing_key": "media",
        },
    },
...
def download(self, url, timeout=180):
    ...
    try:
        ...
        # 내려받기 시작을 로그에 출력
        logger.info(
            "[download start] sleep: {time} {file_name}".format(
        ...
        logger.error(
            "[download error - retry] file: {file_name}, e: {e}".format(
